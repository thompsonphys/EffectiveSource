"""Test that the context-based API produces identical results to the global API."""
import math
import effsource_circular as es
...
E = math.sqrt(1 - 2*M/r_p + a**2/r_p**2 - 2*M*a**2/r_p**3) / math.sqrt(1 - 3*M/r_p + 2*a*math.sqrt(M/r_p**3))
L = (r_p**2 - 2*M*r_p + a*math.sqrt(M*r_p)) / (r_p * math.sqrt(r_p - 3*M + 2*a*math.sqrt(M/r_p)))
ur_p = 0.0
...
ctx1 = es.EffsourceContext(M, a)
ctx2 = es.EffsourceContext(M, 0.9)  # different spin
...
ctx2.set_particle(x_p2, E2, L2, ur_p)
